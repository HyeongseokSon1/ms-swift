                ...
                if args.use_vllm:
                    ...
                    if teacher_data is not None:
                        ...
                        with self._template_context(self.template):
                            encoded_inputs['_opsd_teacher_inputs'] = self._prepare_batch_inputs(
                                teacher_data, encode_prompt_only=False)
                        # The teacher sequence is longer than the student's (it carries the
                        # demonstration prompt), but compute_loss builds teacher_fwd_inputs by
                        # inheriting the student's (shorter) position_ids before overwriting with
                        # the teacher inputs. In standard (non padding-free) mode data_collator
                        # does not emit position_ids, so without this the student's position_ids
                        # leak through and trigger a rotary embedding size mismatch. Derive the
                        # teacher's own position_ids from its attention_mask to keep them aligned.
                        teacher_inputs = encoded_inputs['_opsd_teacher_inputs']
                        if 'position_ids' not in teacher_inputs and 'attention_mask' in teacher_inputs:
                            teacher_position_ids = teacher_inputs['attention_mask'].cumsum(dim=1) - 1
                            teacher_position_ids[teacher_position_ids < 0] = 0
                            teacher_inputs['position_ids'] = teacher_position_ids
